...
* Within the frame of a *data.table*, columns can be referred to *as if they are variables*. Therefore, we simply refer to `origin` and `month` as if they are variables. We do not need to add the prefix `flights$` each time. However using `flights$origin` and `flights$month` would work just fine.
...
* A comma after the condition is also not required in `i`. But `flights[origin == "JFK" & month == 6L, ]` would work just fine. In *data.frames* however, the comma is necessary.
